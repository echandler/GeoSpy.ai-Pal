...
(function() {
    'use strict';
    const state = {_id: 0, duelsLastRoundNum: null, infoWindowDefaultClosed: false,};
...
//state.AI_PLAYER.rounds.pop();
    ...
    function _showAIGuess_standard_finalResultsPage(){
        ...
        function updateScoreBoard(){
            AI_layout.childNodes.forEach((el, idx) => {
                ...
                roundInfoEL.innerHTML = `${distance} ${unit}`;
            });
    ...
    function showAIGuess_marker(curGuess, dragEndCb, dontChangeBounds, showInfoWindowOnCreate){
        ...
        if (!state.infoWindowDefaultClosed && curGuess.infoWindowOpened === undefined && showInfoWindowOnCreate) {
            // Show infowindow for a few seconds.
        ...
        curGuess.marker.addListener('click', () => {
            ...
            if (curGuess.infoWindowOpened) {
                infoWindow.open({ anchor: curGuess.marker })
                state.infoWindowDefaultClosed = false;
                shootTarget(curGuess.marker.getPosition().toJSON(), curGuess.svPos);
            } else {
                state.infoWindowDefaultClosed = true;
                infoWindow.close();
            }
        });
    ...
    function saveRounds(){
        ...
        ls[mapId] = {rounds: rounds, savedDate: Date.now()};
    ...
    function cleanLocalStorage(){
        // Discard old rounds so that localstorage doesn't get filled up.
        const ls = localStorage["aipal"]? JSON.parse(localStorage["aipal"]) : {};
        const obj = {}; 

        let vals = Object.entries(ls);
        vals = vals.filter(val => val[1].savedDate);// Removed any that don't have a savedDate.
        vals = vals.sort((a,b)=> b[1].savedDate - a[1].savedDate);
        vals.length = 50;

        vals.forEach(val => obj[val[0]] = val[1]);

        localStorage["aipal"] = JSON.stringify(obj);
    }
...
*/
    ...
    async function getCoords(resToJSON, country){
        ...
        if (latLng && Array.isArray(latLng) && !isNaN(parseFloat(latLng[0])) && !isNaN(parseFloat(latLng[1]))){
